...
export function EmptyState({ onImport, onCreate }: Props) {
  return (
    <div className="flex flex-col gap-4 py-2">
      <div>
        <p className="text-sm font-semibold text-stone-100">まだ1本も記録が無い</p>
      ...
      <div className="flex flex-wrap gap-x-2 gap-y-2">
        <button
          type="button"
          onClick={onImport}
          className="whitespace-nowrap rounded border border-stone-300 bg-stone-200 px-3 py-1.5 text-sm font-medium text-stone-900"
        >
          JSON を取り込む
        </button>
        <button
          type="button"
          onClick={onCreate}
          className="flex items-center gap-1.5 whitespace-nowrap rounded border border-stone-700 px-3 py-1.5 text-sm text-stone-100"
        >
      ...
      <div className="flex flex-col gap-1.5 text-xs leading-relaxed text-stone-400">
        <p>
          <span className="text-stone-300">取り込み</span>
          ： エクスポートした JSON か、既存の台帳から作った JSON を読む。1行でも形が違えば1件も保存せず理由を出す。
        </p>
        <p>
          <span className="text-stone-300">記録</span>
