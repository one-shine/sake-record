...
// 告示 第5項「任意記載事項」の定義（逐語）。
//
// ★ **この2つが在ることが要点。** 「特定名称の表に無い＝法令に定義が無い」は**誤り**で、
// `原酒` `生酒` は第5項に定義がある。スペック欄の語の出所を判定するとき、
// 第1項の表だけを見て「定義が無い」と結論してはいけない
// （`Learn.test.tsx` が `原酒` に定義が付くことを固定している。逆向きの回帰を止めるため）。
//
// 第5項には他に 生貯蔵酒 / 生一本 / 樽酒 / 貯蔵年数 もあるが**ここには写さない** —
// `styleTermOrigin.ts` の判定に要る2語だけを置き、第5項の一覧を名乗らせない。

/** 告示 第5項(4)。`styleTermOrigin.ts` が `原酒` と `生原酒` の両方で引く */
export const GENSHU_DEFINITION =
  '製成後、加水調整（アルコール分1%未満の範囲内の加水調整を除く。）をしない清酒である場合に表示できる。'

/** 告示 第5項(5)。`生原酒` の判定に要るのでここに置く（`生酒` 単体は `STYLE_TERMS` に無い） */
export const KIZAKE_DEFINITION = '製成後、一切加熱処理をしない清酒である場合に表示できる。'
