...
//
// ## 数値は CI が同梱データと突き合わせる
//
// 語彙・銘柄数・打ち切りの段差・上位5語の割合は**リテラルだが、`facts.test.ts` が同梱 JSON を
// 数え直して一致を要求する**。月次の `update-sakenowa.yml` は commit の前に `npm run test` を
// 通すので、上流が動いてこの表とずれたらそこで赤になる（黙って画面の数字だけ古くならない）。
// **赤にしたら、実測に合わせてここの値を直す**（勝手に自動更新はしない = 変化に人が気付く）。
//
// ## 割合は「銘柄に対する割合」
//
