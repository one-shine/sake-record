...
//
// 検出しているのは「勾配(隣との差)が濃いセルの最大連結成分」= 文字らしい領域。
...
describe('文字らしい領域の検出', () => {
  ...
  it('勾配が弱すぎる画像も null(ノイズの相対比較で枠を作らない)', () => {
    // 差2の縞 = セル密度 約4 < DETECT_MIN_PEAK(10)。門が無いと「最大に対する比」だけで
    // ノイズが枠に化ける(ぼけた写真で起きる)
    const gray = flat()
    for (let y = 40; y < 80; y++) {
  ...
  it('文字が画面いっぱいなら null(枠を切る意味が無い)', () => {
    expect(findTextRegion(paintStripes(flat(), 0, 0, 128, 128), 128, 128)).toBeNull()
  })
  ...
  it('調整済みの定数(変えるときは gray.json の走査で測り直す)', () => {
    ...
    expect(DETECT_MIN_PEAK).toBe(10)
