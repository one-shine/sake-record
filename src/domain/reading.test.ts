...
describe('find — OCR が読んだ文字列', () => {
  ...
  it('読みの合間に雑音が1文字割り込んでも当たる — 端末で1文字ずれた実例', () => {
    // 同じ写真・同じ実装で、卓上は `みやいずみ` / 実機(iOS)は `みやゃいずみ` と読んだ。
    // **完全一致だけだと端末が変わっただけで落ちる**
    const hits = index.find('ヘンバンpremiumsake みやゃいずみc。泊っ。')
    expect(hits.map((h) => h.brandId)).toEqual([1])
    // 画面には**読んだ通り**を返す(`ミヤイズミ` と偽らない)
    expect(hits[0].reading).toBe('ミヤャイズミ')
  })

  it('2文字割り込んだら採らない(1文字までしか許さない)', () => {
    expect(index.find('みやゃゅいずみ')).toEqual([])
  })

  it('読み飛ばした雑音は読みの長さに数えない', () => {
    // `デンシュ`(4文字) は門に1文字足りない。雑音を1つ挟んで5文字に見せかけても通らない
    expect(index.find('ササでんっしゅココ')).toEqual([])
  })
})
