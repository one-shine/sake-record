 ...
 * 空セルは半角スペース2つ。表/裏ラベルで内容が完全に同じ2組を含む(2025-12-08 赤武 /
 * 2025-12-12 加茂錦 — この2組は PHASE_2 の完了条件そのもの)。
 */
const THREE_TABLES = [
  '# 飲酒ログ',
  '',
  ...
  HEADER,
  DIVIDER,
  '| 1 | 2025-12-08 | 赤武 | 岩手県 | 純米酒 |  |',
  '| 2 | 2025-12-08 | 赤武 | 岩手県 | 純米酒 |  |',
  '| 3 | 2025-12-12 | 加茂錦 | 新潟県 |  | 表ラベル |',
  '| 4 | 2025-12-12 | 加茂錦 | 新潟県 |  | 裏ラベル |',
  '| 5 | 2026-01-03 | 不明 |  |  |  |',
  '',
  '## 次の節',
  '',
  '| 9 | 2026-02-01 | 節の外の酒 | 東京都 |  |  |',
  '',
].join('\n')
...
describe('`## 全記録` 節だけを食う', () => {
  it('前後の表を取り込まない(見出しがアンカー / 節は次の `##` で終わる)', () => {
    ...
    expect(rows.map((row) => row.brandLabel)).not.toContain('福島県')
    // 節の外(次の見出し以降)の行も食わない
...
describe('内容が同じ行を dedupe しない(PHASE_2 の完了条件)', () => {
  // 表/裏ラベルの写真が別々に1本として数えられている。素朴な dedupe を入れると
  // 203本が静かに201本になり、しかも「件数が合わない」以外の症状が出ない。
  it('(2025-12-08, 赤武) と (2025-12-12, 加茂錦) が各2件として残る', () => {
    const { rows } = parseSakeLog(THREE_TABLES)
    const same = (drankOn: string, brandLabel: string) =>
      rows.filter((row) => row.drankOn === drankOn && row.brandLabel === brandLabel)

    expect(same('2025-12-08', '赤武')).toHaveLength(2)
    expect(same('2025-12-12', '加茂錦')).toHaveLength(2)
    // 区別できるのは No. だけ(日付・銘柄・県・スペックは完全に同じ)
    expect(same('2025-12-08', '赤武').map((row) => row.no)).toEqual([1, 2])
    expect(rows).toHaveLength(5)
  })
