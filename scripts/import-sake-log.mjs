 ...
 *
 * ## このスクリプトは CI で一度も実行されない(BACKLOG B23。隠さず書く)
 *
 * 入力(`../brain/40_Resources/sake-log.md`)がリポジトリ外の個人データなので、GitHub Actions
 * からは到達できない。**`npm run ci` にも月次更新ジョブにも入っていない。** つまり:
 *   - 実データ側の期待値(203件 / `(日付, 銘柄)` の重複が2組)を確かめるのは**手元での実行だけ**
 *   - この殻が壊れても CI は緑のまま。気づくのは次に取り込もうとしたとき
 * 守れる範囲は分けてある: パースの構造不変条件は `parseSakeLog.test.ts`(CI で走る)、
 * 射影の混入規則は `linkBrand.test.ts` と `npm run ledger:check`(CI で走る)。
 *
 * Node 版だけは宣言と検査で守る: `package.json` の `engines.node` に
 * `>=22.18.0 <23.0.0 || >=23.6.0` を書き、`npm run node:check`(= `npm run ci` の先頭)が
 * **走っている Node と `.github/workflows/*.yml` の `node-version` の両方**を照合する。
 * ランナーの Node を下げてもこのスクリプトは CI で走らないので気づけない — だから宣言を検査する。
 *
