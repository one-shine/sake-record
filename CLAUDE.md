...
- **OCR の律速は認識ではなく検出・分割。** 銘柄の列だけを正しく切り出せば `jpn` PSM6 で 12/19 字・4/9 銘柄まで読める（倍率には鈍感で、文字高の正規化は梃子にならない）。**テキスト行の検出をヒューリスティクスで自作すると逆に悪化する**（列の連結成分で分割 → 4/19。実測）。tesseract は**文書OCR**で、曲面・斜め・装飾書体のラベルは研究上**シーンテキスト認識**という別問題 — 段差のある改善は engine の載せ替えしかない（→ B67。資産 7.7MB → 25〜30MB で SPEC の上限を超えるので仕様判断が要る）。
