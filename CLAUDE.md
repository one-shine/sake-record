...
- **精度が足りないときに緩めるのは候補の門ではなく「人が押す道」。** 候補の閾値（希少性・被覆率）を下げると増えるのは当たりではなく当てずっぽう（実測で否定済み）。代わりに**読めた字を絞り込みの鍵として押せる形で出す**（`createCharNarrower` → 押した先は手動サジェストと同じ一覧）。鍵は**信頼度で照合から外した分も含めた読めた全部**から作る（押すのは人なので勝手に1位が出ない）。実測 9枚で候補 3 + 1タップ 3 = 6枚に到達。
