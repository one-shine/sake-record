...
- **文字の場所は「自動で提案 → 外れたら人が囲む」の2段。** tesseract のレイアウト解析は画面いっぱいの文書が前提で、**瓶の全体が写る写真からラベルを見つけられない**（瓶全体の合成9枚で到達 4/9。実機では「全く読み取れない」ように見える）。`src/lib/ocr/findLabel.ts` が**文字密度（勾配の濃いセルの最大連結成分）**でラベルらしい範囲を提案し、切り出して読む。**候補が出なければ全体も読んで合算**（悪化を構造的に防ぐ）。これが「銘柄を推測しない」規律と衝突しないのは**場所の提案であって内容の判定ではない**から — どの経路でも候補の門と選ぶ人は同じで、読んだ範囲は画面に明示し、枠は「ラベルを囲んで読み取る」で人が引き直せる（自動の枠が初期値に入る）。囲んだ範囲は原寸から切り出し（`cropImage.ts`。枠は表示比率 0..1 で受け渡す）同じ認識経路に流す。**手動の切り出しに失敗したら全体に黙って落とさない**（枠が効いていないように見える）。実測（ボタン1回）: シーン 4/9→**7/9** / 写真ふう 6/9→**7/9** / clean 7/9→**8/9**。手動で狭く囲み直すとシーン 8/9。
